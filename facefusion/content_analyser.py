...
def analyse_video(video_path: str, start_frame: int, end_frame: int) -> bool:
    ...
    rate = 0.0
    counter = 0
    last_successful_frame = None  # 最後に成功したフレームを保存
        ...
        for frame_number in frame_range:
            if frame_number % int(video_fps) == 0:
                try:
                    ...
                    if frame is None:
                        raise ValueError(f"Frame {frame_number} could not be retrieved")
                    last_successful_frame = frame  # 成功したフレームを保存
                except Exception as e:
                    print(f"Error processing frame {frame_number}: {e}")
                    if last_successful_frame is not None:
                        print(f"Using last successful frame for frame {frame_number}")
                        frame = last_successful_frame  # 失敗した場合、前回のフレームを使用
                    else:
                        continue  # 失敗し、前回のフレームもない場合はスキップ
